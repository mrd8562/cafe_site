...
// Обработка возврата из bePaid по return_url/success_url (query: status, uid, token...)
(function handleBePaidReturn() {
    const run = () => {
        try {
            const params = new URLSearchParams(window.location.search || '');
            const status = (params.get('status') || '').toLowerCase();
            const trackingFromUrl = params.get('tracking_id') || params.get('trackingId') || '';
            const trackingFromStorage = (() => { try { return localStorage.getItem('bepaid_last_tracking') || ''; } catch { return ''; } })();
            const trackingId = trackingFromUrl || trackingFromStorage;

            if (!trackingId || !trackingId.includes('cafe180_')) return false;
            if (status !== 'successful' && status !== 'paid' && status !== 'success') return false;

            const apiBase = (() => { try { return localStorage.getItem('bepaid_api_base') || ''; } catch { return ''; } })();
            const confirmUrl = apiBase ? `${apiBase}/api/bepaid/confirm` : '/api/bepaid/confirm';
            ...
            const canUi = (typeof openSuccessModal === 'function') && (typeof clearFoodOrders === 'function');
            if (canUi) {
                openSuccessModal();
                clearFoodOrders();
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        const t = setInterval(() => {
            tries += 1;
            if (run() || tries >= 40) clearInterval(t);
        }, 250);
    });
})();
