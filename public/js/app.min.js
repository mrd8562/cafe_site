              ...
              <div class="tmod">
                ...
                <div class="tmod__options">
                  ...
                  <div class="topt">
                    ...
                    <div class="topt__name">
                      <span class="topt__name-wrap">Перец Халапеньо, 5&nbsp;г</span> <span class="topt__price">+1&nbsp;руб.</span>
                    </div>
                    <div class="topt__checkbox"></div>
                  </div>
                </div>
...
document.addEventListener('click', function (e) {
    if (e.target.closest('.header__user.header__user--anonym')) {
        ...
        const description = JSON.parse(dishElement.dataset.description);
...
function validateOrderData(data) {
    ...
    if (data.deliveryType === 'Предзаказ') {
        ...
        if (!/^\d{2}:\d{2}/.test(data.preorderTime || '')) errs.push({ field: 'preorderTime', message: 'Выберите время' });
    }
              ...
              <div class="cart">
                <div class="csuccess">
                  ...
                  <div class="csuccess__content">
                    <div class="csuccess__text">Спасибо! Мы свяжемся с вами в ближайшее время.</div>
                  </div>
                </div>
              </div>
            </div>
...
function addDishToOrder(button) {
    ...
    const orderItem = {
        dish: {
            ...
            imageUrl: dishData.imageUrl // Добавляем ссылку на изображение
        },
...
function initCart() {
    ...
    if (orders.length === 0) {
        ...
        orders.forEach((order, index) => {
            ...
            // Формируем список начинок
            let toppingsText = '';
            if (toppings.length > 0) {
                toppingsText = toppings.map(topping => {
                    return topping.quantity > 1 ?
                        `${topping.name} (${topping.quantity})` :
                        topping.name;
                }).join(', ');
            }

            // Считаем общую стоимость блюда (цена уже включает начинки)
                  ...
                  <div class="pdish" data-index="${index}">
                      <div class="pdish__content">
                          <div class="pdish__name">
                              ${dish.name}
                              ${toppingsText ? `<span class="pdish__sub">${toppingsText}</span>` : ''}
                              <div class="pdish__size">${dish.weight} г</div>
                          </div>
    ...
    if (orders.length !== 0) {
        orders.forEach((order, index) => {
          ...
          <div class="cdish cdish--with-photo" data-index="${index}">
            <img src="${dish.imageUrl}" class="cdish__photo">
            <div class="cdish__name">${dish.name}
              ${toppingsText ? `<span class="cdish__sub">${toppingsText}</span>` : ''}
              <span class="cdish__size">${dish.weight} г</span>
            </div>
            <div class="cdish__meta">
              <div class="cdish__controls">
                <div class="cdish__control cdish__control--minus"></div>
...
document.addEventListener('click', function (e) {
    ...
    if (day) {
        ...
        if (!csugg) {
          ...
          <div class="csugg">
            <div class="csugg__scrollable">
              <div data-id="18:30 - 19:00" class="csugg__option">18:30 - 19:00</div>
              <div data-id="18:45 - 19:15" class="csugg__option">18:45 - 19:15</div>
              <div data-id="19:00 - 19:30" class="csugg__option">19:00 - 19:30</div>
              <div data-id="19:15 - 19:45" class="csugg__option">19:15 - 19:45</div>
              <div data-id="19:30 - 20:00" class="csugg__option">19:30 - 20:00</div>
              <div data-id="19:45 - 20:15" class="csugg__option">19:45 - 20:15</div>
            </div>
          </div>
        `;
            wrap.insertAdjacentHTML('beforeend', csuggHTML);
