          ...
          <div class="ch" data-screen="main">
            <div class="ch__screen ch__screen--main">
              <div class="ch__header">
                ...
                <div class="ch__subtitle">Пожалуйста, сообщите<br> нам о проблеме, с которой<br> вы столкнулись, используя<br> сервис cafe180.by.</div>
              </div>
              <a href="tel:375293798989" class="ch__phone-wrapper">
                <div class="ch__phone">
                  <div class="ch__phone-number" data-before="Тел. моб.:" data-after="A1, MTC">379-89-89</div>
              ...
              <div class="ch__form">
                ...
                <label class="сh__policyWrap">
                  ...
                  <div class="ch__boxLabel">Я принимаю условия <a class="ch__boxLink" href="/" target="_blank">соглашения</a> 
                    и правила обработки <a class="ch__boxLink" href="/" target="_blank">персональных данных</a>
                  </div>
            ...
            <div class="ch__screen ch__screen--success ch__success">
              ...
              <div class="ch__success-message">Служба поддержки клиентов<br> cafe180.by работает с 08:55 до 01:00.<br> Мы получили сообщение о проблеме<br> и свяжемся с вами как можно раньше.</div>
            </div>
          </div>
        </div>
      </div>
...
function initReportForm(modalRoot) {
    const container = modalRoot || document.querySelector('.ch');
    if (!container) return;

    const form = container.querySelector('.ch__form');
    const nameInput = form?.querySelector('.ch__input--name');
    const phoneInput = form?.querySelector('.ch__input--phone');
    const messageInput = form?.querySelector('.ch__input--txt');
    const policyCheckbox = container.querySelector('.ch__boxCheckbox');
    const policyWrap = container.querySelector('[class*="policyWrap"]');
    const submitButton = form?.querySelector('.ch__submit');

    if (!form || !nameInput || !phoneInput || !messageInput || !policyCheckbox || !submitButton) {
        return;
    }

    let statusNode = form.querySelector('.ch__form-alert');
    if (!statusNode) {
        statusNode = document.createElement('div');
        statusNode.className = 'ch__form-alert';
    ...
    const setStatus = (text = '', type = 'error') => {
        ...
        statusNode.style.color = type === 'success' ? '#2e7d32' : '#e74c3c';
    };
    ...
    const setPolicyError = (hasError) => {
        if (!policyWrap) return;
        const box = policyWrap.querySelector('.ch__box');
        if (box) {
            box.style.borderColor = hasError ? '#ff4d4f' : '';
        }
        if (hasError) {
            policyWrap.classList.add('ch__policy--error');
        } else {
            policyWrap.classList.remove('ch__policy--error');
        }
    };
    ...
    const resetFields = () => {
        ...
        if (policyWrap) {
            policyWrap.classList.remove('ch__policy--selected', 'ch__policy--error');
            const box = policyWrap.querySelector('.ch__box');
    ...
    const toggleLoading = (isLoading) => {
        ...
        submitButton.classList.toggle('btload--loading', Boolean(isLoading));
    };
    ...
    policyCheckbox.addEventListener('change', () => setPolicyError(false));
    ...
    submitButton.addEventListener('click', async (event) => {
        ...
        if (name.length < 2) {
            ...
            firstError = 'Введите имя (не менее 2 символов).';
        }

        if (!firstError && phoneDigits.length < 7) {
            setInputError(phoneInput);
            firstError = 'Введите корректный номер телефона.';
        }

        if (!firstError && message.length < 10) {
            setInputError(messageInput);
            firstError = 'Опишите проблему подробнее (минимум 10 символов).';
        }

        if (!policyCheckbox.checked) {
            setPolicyError(true);
            if (!firstError) {
                firstError = 'Необходимо принять условия обработки персональных данных.';
            }
        }
        ...
        try {
            const payload = {
                ...
                phone,
                message,
                acceptedPolicy: policyCheckbox.checked,
            ...
            if (!response.ok || !data.success) {
                throw new Error(data.message || 'Не удалось отправить сообщение. Попробуйте позже.');
            }

            setStatus('Спасибо! Мы уже получили сообщение.', 'success');
            container.dataset.screen = 'success';
            resetFields();
        } catch (error) {
            const messageText = error?.message || 'Произошла ошибка. Повторите попытку позже.';
                ...
                <div class="adelivery">
                  ...
                  <div class="adelivery__field adelivery__field--3">
                    <div class="adelivery__col">
                      <label class="ainput ainput--filled">
                  ...
                  <div class="adelivery__field adelivery__field--comment">
                    <label class="ainput ainput--filled" autogrow="true">
...
//                 <div class="cextra__navs">
//                   <div class="cextra__nav cextra__nav--prev"></div>
//                   <div class="cextra__nav cextra__nav--next"></div>
//                 </div>
...
function openCityDropdown() {
    closeCityDropdown();
    const cityButton = document.querySelector('.header__nav.header__nav--city');
    const headerNavs = document.querySelector('.header__navs');
    if (!cityButton || !headerNavs) return;
    headerNavs.insertAdjacentHTML('beforeend', cityDropdownHTML);
    const dropdown = headerNavs.querySelector('[data-tippy-root]');
    const rect = cityButton.getBoundingClientRect();
    const headerNavsRect = headerNavs.getBoundingClientRect();
    dropdown.style.top = (rect.top - headerNavsRect.top) + 'px';
    dropdown.style.left = (rect.left - headerNavsRect.left) + 'px';
    dropdown.querySelectorAll('.tdrop__option').forEach(option => {
        option.addEventListener('click', function () {
            const headerName = document.querySelector('.header__nav-name');
            if (headerName) headerName.textContent = this.textContent;
            closeCityDropdown();
        });
        option.addEventListener('mouseenter', function () {
            this.classList.add('tdrop__option--highlight');
        });
        option.addEventListener('mouseleave', function () {
            this.classList.remove('tdrop__option--highlight');
        });
    });
    setTimeout(() => document.addEventListener('click', closeCityDropdownOnClickOutside), 0);
}
...
function closeCityDropdownOnClickOutside(e) {
    const headerNavs = document.querySelector('.header__navs');
    const cityButton = document.querySelector('.header__nav.header__nav--city');
    const dropdown = headerNavs?.querySelector('[data-tippy-root]');
    if (dropdown && !dropdown.contains(e.target) &&
        (!cityButton || !cityButton.contains(e.target))) {
        closeCityDropdown();
    }
}

document.addEventListener('click', function (e) {
    if (e.target.closest('.header__user.header__user--anonym')) {
    ...
    else if (e.target.closest('.header__nav.header__nav--problem')) {
        openModal(reportHTML);
        const layouts = document.querySelectorAll('.vr__layout');
        const currentLayout = layouts[layouts.length - 1];
        const reportModal = currentLayout?.querySelector('.ch');
        if (reportModal) {
            const checkboxElements = reportModal.querySelectorAll('.ch__boxCheckbox');
            checkboxElements.forEach(function (checkbox) {
                checkbox.addEventListener('click', function () {
            ...
            initReportForm(reportModal);
        }
    }
    else if (e.target.closest('.header__nav-name.header__phone')) {
    ...
    else if (e.target.closest('.ddish__photos') || e.target.closest('.ddish__plus') || e.target.closest('.ddish__name')) {
      e.preventDefault();
      ...
      const preorder = (() => {
          ...
          const timeRaw = formEl.querySelector('.cpre__time-input')?.textContent || '';
      ...
      if (data.paymentType === 'Картой') {
        (async () => {
            try {
                const resp = await fetch('/api/order/attempt', {
                    method: 'POST',
                ...
                if (!resp.ok || !j?.success) {
                    // Не открываем виджет, если не смогли отправить "попытку оплаты"
                    showInlineFormError(j?.message || 'Не удалось отправить уведомление. Попробуйте ещё раз.');
                    return;
                }
                initBePaidPayment(data);
            } catch (e) {
                showInlineFormError('Ошибка сети. Проверьте подключение.');
            }
        })();
      } else if (data.paymentType === 'Наличными') {
        submitOrderToServer(data);
        openSuccessModal();
        clearFoodOrders();
      } else {
        showInlineFormError('Выберите способ оплаты');
        return;
      }
  }
    ...
    else if (e.target.closest('.header__nav.header__nav--city')) {
        openCityDropdown();
        e.stopPropagation();
    }
});
...
function initBePaidPayment(orderData) {
    let orderSubmitted = false;

    // Генерация/получение device_id
    let deviceId = localStorage.getItem('bepaid_device_id');
    if (!deviceId) {
        deviceId = Array.from(crypto.getRandomValues(new Uint8Array(16)))
            .map(b => b.toString(16).padStart(2, '0'))
            .join('');
        localStorage.setItem('bepaid_device_id', deviceId);
    }

    const amountMinor = Math.round(orderData.total * 100);
    const trackingId = `cafe180_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
    ...
    const paymentParams = {
        ...
        checkout: {
            iframe: true,
            public_key: "",
            shop_id: 33460,
            transaction_type: "payment",
            order: {
                currency: "BYN",
                amount: amountMinor,
                description: `Заказ от ${orderData.name} (${orderData.phone})`,
                ...
                additional_data: {
                    ...
                    // Убран vendor — он не нужен без агрегатора
                },
                expired_at: null
            },
            settings: {
                ...
                fail_url: null,
                decline_url: null,
                notification_url: "https://cafe180.by/api/bepaid/webhook", // ← ВАЖНО: реальный URL!
                ...
                save_card_toggle: {
                    display: true,
                    customer_contract: true // ← вы говорили, что хотите включено по умолчанию — это правильно
                },
                another_card_toggle: {
                    display: true
                },
            ...
            customer: {
                first_name: orderData.name.split(' ')[0] || null,
                last_name: orderData.name.split(' ').slice(1).join(' ') || null,
                address: orderData.address || null,
                city: orderData.city || "Новополоцк", // ← можно брать из формы
                country: "BY", // ← лучше явно указать
                state: null,
            ...
            // УДАЛЕНЫ: finished, expired, status, message, version, card_info, job_id, payment_method

            shop: {
                id: "33460",
                name: "Cafe 180",
                country: "BY",
                url: "https://cafe180.by",
                contact_email: "cafe180@gmail.com",
                contact_phone: "+375293798989",
                brands: ["visa", "mastercard", "belkart", "mir", "gpay", "samsungpay"]
            },
            merchant: {
                id: "1253", // ← если не меняли в ЛК — оставьте; иначе уточните
                country: "BY"
            },
            test: true // ← ✅ главное: теперь реальный режим
        },
        closeWidget: function(status) {
            ...
            if (status === 'successful' || status === 'paid') {
                orderSubmitted = true;
                // Оформляем заказ и шлём 2 сообщения на бэке через /api/order (paid=true)
                submitOrderToServer(Object.assign({}, orderData, { paid: true, trackingId }));
                openSuccessModal();
                clearFoodOrders();
                return;
            }

            // При проваленной операции показываем ошибку под "Итого" и не закрываем наше окно оформления
            if (status === 'failed' || status === 'declined' || status === 'error') {
                showPaymentInlineError('Операция не прошла. Попробуйте ещё раз.');
                return;
            }
        }
    };

    new BeGateway(paymentParams).createWidget();
}
...
function showPaymentInlineError(message) {
    try {
        const formEl = document.querySelector('.csform');
        if (!formEl) return;
        const totalBlock = formEl.querySelector('.cfooter__total');
        if (!totalBlock) return;

        // Удаляем старую ошибку, если была
        const prev = formEl.querySelector('.ctinput__error.ctinput__error--payment');
        if (prev) prev.remove();

        const div = document.createElement('div');
        div.className = 'ctinput__error ctinput__error--payment';
        div.style = 'margin-bottom: 10px; padding-left: 7px; line-height: 1.2;';
        div.innerHTML = `${message} Можно обратиться по телефону <a style="color: red; text-decoration: underline;" href="tel:+375293798989">+375 (29) 379-89-89</a>`;
        totalBlock.insertAdjacentElement('afterend', div);
    } catch (e) {}
}
...
function validateOrderData(data) {
    ...
    if (data.type.includes('Доставка')) {
        ...
        if (itemsSum < 40) {
            errs.push({ field: 'items', message: 'Минимальный заказ для доставки — 40 руб.' });
        }
    }
...
function clearFormErrors(formEl) {
    ...
    // Удаляем возможную ошибку из футера (для ошибок корзины)
...
function renderFormErrors(formEl, errors) {
    errors.forEach(err => {
        if (err.field === 'name') {
            ...
            // Покажем ошибку минимальной суммы заказа во футере формы
...
function addDishToOrder(button) {
    ...
    // Для каждой начинки создаем отдельную позицию в корзине (с учетом количества)
...
function extractToppingsData(container) {
    ...
    selectedToppings.forEach(topping => {
        ...
        if (nameElement) {
            name = nameElement.textContent.trim(); // сохраняем граммовку в названии
        }
    ...
    if (orders.length === 0) {
        pcart.innerHTML = `
              ...
              <div class="pcart__dishes">
                  <div class="pcart__empty">Выберите блюда<br> и добавьте их к заказу</div>
              </div>
              <div class="pcart__footer">
                  <div class="pfoot" style="display: none;">
                      <div data-text="Оформить заказ" class="pfoot__submit btload btload--white">
                          <i class="btload__ico"></i>
                          <i class="btload__ico"></i>
                          <i class="btload__ico"></i>
                      </div>
        ...
        orders.forEach((order, index) => {
            ...
            dishesHTML += `
                  <div class="pdish" data-index="${index}">
                      <div class="pdish__content">
                          <div class="pdish__name">
                              ${displayName}
                              ${displaySize ? `<div class=\"pdish__size\">${displaySize}</div>` : ''}
                          </div>
                          <div class="pdish__quantity">
                              <div class="pdish__button pdish__button--minus"></div>
                              <div class="pdish__num">${dish.quantity}</div>
                              <div class="pdish__button pdish__button--plus"></div>
                          </div>
                          <div class="pdish__price">${dishTotalPrice.toFixed(2)}&nbsp;руб.</div>
                      </div>
                  </div>
              `;
        });

        // Определяем начальное состояние табов на основе сохраненного значения
        const isDelivery = currentTabState === 'delivery';

        pcart.innerHTML = `
              <div class="pcart__header">
                  <div class="phead">
                      <div class="phead__title">Мой заказ</div>
                      <svg width="19px" height="20px" viewBox="0 0 19 20" class="phead__trash">
                          <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" stroke-linecap="round">
                              <g transform="translate(-1490.000000, -178.000000)" stroke="#000000" stroke-width="2" class="phead__trash-g">
              ...
              <div class="pdv">
                  <div class="pdv__item ${isDelivery ? 'pdv__item--active' : 'pdv__item--hoverable'}" style="width: calc(50%);">
                      <div class="pdv__name">Доставка</div>
                      <div class="pdv__pointer" style="transform: ${isDelivery ? 'translateX(0%)' : 'translateX(100%)'};"></div>
                  </div>
                  <div class="pdv__item ${isDelivery ? 'pdv__item--hoverable' : 'pdv__item--active'}" style="width: calc(50%);">
                      <div class="pdv__name">Навынос</div>
                  </div>
              </div>
              <div class="pcart__dishes">
                  <div class="pfee" style="display: none;">
                      <div class="pfee__right" style="display: none;">
                          <div class="pfee__action">Указать адрес</div>
                      </div>
                      <div class="pfee__left">
                          <div class="pfee__title" style="display: none;">Доставка</div>
                          <div class="pfee__text" style="display: none;">Укажите свой адрес,<br>чтобы рассчитать доставку</div>
                      </div>
                  </div>
                  <div class="pdishes">
                      ${dishesHTML}
              ...
              <div class="pcart__footer">
                  <div class="pfoot" style="">
                      <div class="pfoot__total">
                          <div>Итого</div>
                          <div class="pfoot__amount">${totalAmount.toFixed(2)}&nbsp;руб.</div>
                      </div>
                      <div data-text="Оформить заказ" class="pfoot__submit btload btload--white">
                          <i class="btload__ico"></i>
                          <i class="btload__ico"></i>
...
function openDelivery() {
    ...
    if (orders.length !== 0) {
        ...
        // Рассчитываем доставку: бесплатно от 40 руб., иначе 8 руб.
        const deliveryFee = totalAmount >= 50 ? 0 : 6;
        ...
        <div class="csform">
          ...
          <div class="cfooter">
            <div class="cfooter__total">
              <div class="cfooter__meta">${deliveryFee === 0 ? 'Доставка бесплатно от 50 руб.' : 'Стоимость доставки — 6 руб.'}</div>
            </div>
            <div class="cfooter__total">
              <div class="cfooter__left">Итого</div>
...
document.addEventListener('click', function (e) {
    if (e.target.closest('.ctmerch .cheader .cheader__back')) {
        openDelivery();
    }
});
...
function initNavigation() {
    ...
    if (!nav) return;
    if (nav.classList.contains('cnav--split')) return;

    const navOverflow = nav.querySelector('.cnav__overflow');
    const navScrollable = nav.querySelector('.cnav__scrollable');
    const navItems = navScrollable.querySelectorAll('.cnav__item');
    const navMore = nav.querySelector('.cnav__item--more');
    const sections = document.querySelectorAll('.nav__anchor');
    const cursor = navOverflow.querySelector('.cnav__cursor');

    let dropdown = null;
    let dropdownElement = null;

    // Функция для создания выпадающего меню внутри cnav__overflow
    const createDropdown = (items) => {
        ...
        dropdownElement.className = 'tippy-box';
        dropdownElement.setAttribute('data-theme', 'dropdown nav-more');
        dropdownElement.setAttribute('data-placement', 'bottom-end');
        dropdownElement.style.cssText = 'position: absolute; z-index: 9999; visibility: hidden; opacity: 0; transition: opacity 0.3s ease;';

        const content = document.createElement('div');
        content.className = 'tippy-content';

        const dropContent = document.createElement('div');
        dropContent.className = 'tdrop';

        items.forEach((item, index) => {
            const option = document.createElement('div');
            option.className = 'tdrop__option';
            option.setAttribute('data-item', index);
            option.textContent = item.name;
            option.dataset.section = item.section;
        ...
        content.appendChild(dropContent);
        dropdownElement.appendChild(content);

        // Добавляем dropdown внутрь cnav__overflow
        navOverflow.appendChild(dropdownElement);
        ...
        return {
            ...
            show: function () {
                ...
                this.element.setAttribute('aria-labelledby', navMore.id || 'more-button');
            },
            hide: function () {
            ...
            updatePosition: function () {
                ...
                const navMoreRect = navMore.getBoundingClientRect();
                const navOverflowRect = navOverflow.getBoundingClientRect();

                // Получаем ширину dropdown
                this.element.style.visibility = 'visible';
                this.element.style.opacity = '0';
                ...
                // Предполагаемые значения для ::after (настройте под ваш дизайн)
                const afterWidth = 10; // Примерная ширина ::after
                const afterRight = 5;  // Примерный отступ ::after справа

                // Правый край кнопки "Ещё" с учетом ::after
                const navMoreRightWithAfter = navMoreRect.right + afterRight + afterWidth;

                // Позиционируем относительно cnav__overflow
                const navMoreRightRelative = navMoreRightWithAfter - navOverflowRect.left;
                const leftPosition = navMoreRightRelative - dropdownWidth + 2;

                // Позиция сверху - нижний край кнопки "Ещё" + offset 8px
                const topPosition = cursorRect.bottom - navOverflowRect.top + 12;

                this.element.style.left = leftPosition + 'px';
                this.element.style.top = topPosition + 'px';
            },
            set: function (newItems) {
                const dropContent = this.element.querySelector('.tdrop');
    ...
    // Функция для обновления aria-expanded
    const updateAriaExpanded = (isExpanded) => {
        navMore.setAttribute('aria-expanded', isExpanded.toString());
    };
    ...
    const handleOutsideClick = (e) => {
        if (dropdown && dropdown.element && !dropdown.element.contains(e.target) && e.target !== navMore && !navOverflow.contains(e.target)) {
            hideDropdown();
        }
    };
    ...
    const initAriaAttributes = () => {
        if (!navMore.id) {
            navMore.id = 'cnav-more-button';
        }

        navMore.setAttribute('aria-haspopup', 'true');
        navMore.setAttribute('aria-expanded', 'false');
        navMore.setAttribute('role', 'button');
        navMore.setAttribute('aria-label', 'Показать дополнительные разделы');
    };
    ...
    navMore.addEventListener('click', (e) => {
        ...
        if (dropdown && dropdown.element.style.visibility === 'visible' && dropdown.element.style.opacity === '1') {
            hideDropdown();
        } else {
            showDropdown();
        }
    ...
    navMore.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            ...
            e.preventDefault();
            showDropdown();
            const firstMenuItem = dropdown.element.querySelector('.tdrop__option');
            if (firstMenuItem) {
                firstMenuItem.focus();
            }
    ...
    // Обработчик изменения видимости элементов
    const onMoreChanged = function (entries) {
        entries.forEach(entry => {
            if (!entry.isIntersecting) {
                entry.target.classList.add('cnav__item--hidden');
            } else {
                entry.target.classList.remove('cnav__item--hidden');
            }
        });

        const hiddenNav = [];
        navItems.forEach(item => {
            if (item.classList.contains('cnav__item--hidden')) {
                hiddenNav.push({
        ...
        if (hiddenNav.length) {
            nav.classList.add('cnav--more');

            if (!dropdown) {
                dropdown = createDropdown(hiddenNav);
            } else {
                dropdown.set(hiddenNav);
            }

            redrawMore();
        } else {
            nav.classList.remove('cnav--more');
            if (dropdown) {
                dropdown.hide();
            }
        }
    ...
    // Функция для перерисовки кнопки "Ещё"
    const redrawMore = () => {
        let maxRightOffset = 0;
        navItems.forEach(item => {
            if (!item.classList.contains('cnav__item--hidden')) {
                maxRightOffset = item.offsetLeft + item.offsetWidth;
            }
        });

        if (document.querySelector('.cnav__item--hidden')) {
            navMore.style.left = '0';
            const navOffset = navMore.offsetLeft - 18;
            navMore.style.left = navOffset > maxRightOffset ? '-' + (navOffset - maxRightOffset) + 'px' : '0px';
        }

        let cursorWidth = 0;
        let cursorOffset = 0;
        let cursorExtraWidth = 0;

        if (document.querySelector('.cnav__item--more.cnav__item--active')) {
            cursorWidth = navMore.offsetWidth;
            cursorOffset = navMore.offsetLeft;
            cursorExtraWidth = 5;
        } else {
            const activeItem = document.querySelector('.cnav__item--active') || document.querySelector('.cnav__item');
            cursorWidth = activeItem.offsetWidth;
            cursorOffset = activeItem.offsetLeft;
        }
        ...
        if (cursorOffset < 10) {
            ...
            cursorWidth += 20 + cursorExtraWidth;
        }
    ...
    const moreObserver = new IntersectionObserver(onMoreChanged, {
        root: navScrollable,
    ...
    const onScroll = function () {
        ...
        for (let i = 0; i < sections.length; i++) {
            const bounds = sections[i].getBoundingClientRect();
            if (bounds.top > 150) {
                break;
            } else {
                current = i;
        ...
        if (prev !== current) {
            ...
            if (navItems[current]) {
                ...
                if (navItems[current].classList.contains('cnav__item--hidden')) {
                    navMore.innerHTML = navItems[current].innerHTML;
                    navMore.classList.add('cnav__item--active');
                    redrawMore();
                } else {
                    navMore.innerHTML = 'Ещё';
                    navMore.classList.remove('cnav__item--active');
                    redrawMore();
                }
            }
            prev = current;
    ...
    const scrollToSection = function (sectionId) {
        const targetSection = document.querySelector('.nav__anchor[data-id="' + sectionId + '"]');
        if (targetSection) {
            window.scrollTo({
    ...
    navScrollable.addEventListener('click', function (e) {
        if (e.target.classList.contains('cnav__item')) {
            scrollToSection(e.target.dataset.section);
        }
    });

    // Обновляем позицию dropdown при ресайзе
...
function handleCpayTypeClick(clickedType) {
    ...
    allTypes.forEach(type => {
        type.classList.remove('copt--active');
    });
    clickedType.classList.add('copt--active');

    const isCard = clickedType.textContent.includes('Карт'); // matches "Картой", "Картой онлайн"
...
// Универсальный обработчик hover для всех элементов tdrop__option
document.addEventListener('DOMContentLoaded', function() {
    function addHoverEffects() {
        const tdropOptions = document.querySelectorAll('.tdrop__option');
        tdropOptions.forEach(option => {
            // Проверяем, не добавлены ли уже обработчики
            if (!option.dataset.hoverAdded) {
                option.addEventListener('mouseenter', function () {
                    this.classList.add('tdrop__option--highlight');
                });
                option.addEventListener('mouseleave', function () {
                ...
                option.dataset.hoverAdded = 'true';
            }
        });
    }
    ...
    // Добавляем эффекты для динамически создаваемых элементов
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach(function(node) {
                    if (node.nodeType === 1) { // Element node
                        const newOptions = node.querySelectorAll ? node.querySelectorAll('.tdrop__option') : [];
...
// Функциональность поиска блюд
document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.querySelector('.header__search-input');
    const searchClear = document.querySelector('.header__search-clear');
    const searchDropdown = document.querySelector('[data-tippy-root]');
    ...
    function showSearchResults(results) {
        ...
        const tdrop = searchDropdown.querySelector('.tdrop');
        if (!tdrop) return;
        
        if (results.length === 0) {
            tdrop.innerHTML = '<div class="search__no-results">Ничего не найдено</div>';
        } else {
            tdrop.innerHTML = results.map((item, index) => `
                <div class="tdrop__option search__item search__item--with-photo" data-item="${index}">
            ...
            tdrop.querySelectorAll('.tdrop__option').forEach((option, index) => {
                option.addEventListener('click', function() {
...
document.addEventListener('click', function (event) {
    const recallButton = event.target.closest('.mp-recall-submit');
    if (!recallButton) return;
    event.preventDefault();
    handleRecallSubmit(recallButton, 'desktop');
});

document.addEventListener('input', function (event) {
    if (!event.target.classList.contains('mp-recall-phone')) return;
    clearRecallFieldError(event.target);
});

const RECALL_MIN_PHONE_LENGTH = 7;

async function handleRecallSubmit(buttonEl, sourceLabel) {
    const wrap = buttonEl.closest('.mp-recall-wrap');
    const phoneInput = wrap?.querySelector('.mp-recall-phone');
    if (!phoneInput) return;

    const validation = validateRecallPhoneValue(phoneInput.value);
    if (!validation.valid) {
        setRecallFieldError(phoneInput, validation.error);
        return;
    }

    clearRecallFieldError(phoneInput);
    showRecallMessage(wrap, '', 'info');
    setRecallButtonLoading(buttonEl, true);

    try {
        await submitRecallRequest({
            phone: validation.raw,
            phoneDigits: validation.digits,
            city: buttonEl.dataset.city || '',
            company: buttonEl.dataset.company || '',
            page: window.location.pathname,
            source: sourceLabel
        });

        phoneInput.value = '+375';
        showRecallMessage(wrap, 'Заявка отправлена! Мы свяжемся с вами в ближайшее время.', 'success');
        setRecallButtonLoading(buttonEl, false);

        const defaultText = buttonEl.dataset.initialText || 'Жду звонка';
...
function validateRecallPhoneValue(value) {
    const normalized = sanitizePhoneValue(value);
    if (normalized.digits.length < RECALL_MIN_PHONE_LENGTH) {
        return { valid: false, error: 'Введите корректный номер телефона.', raw: normalized.raw, digits: normalized.digits };
    }
    return { valid: true, raw: normalized.raw, digits: normalized.digits };
}

function setRecallFieldError(inputEl, message) {
    if (!inputEl) return;
    inputEl.classList.add('mp-recall-phone--error');
    inputEl.setAttribute('aria-invalid', 'true');
    showRecallMessage(inputEl.closest('.mp-recall-wrap'), message, 'error');
}

function clearRecallFieldError(inputEl) {
    if (!inputEl) return;
    inputEl.classList.remove('mp-recall-phone--error');
    inputEl.removeAttribute('aria-invalid');
    const wrap = inputEl.closest('.mp-recall-wrap');
    if (!wrap) return;
    const messageEl = wrap.querySelector('.mp-recall-message');
    if (messageEl && messageEl.dataset.type === 'error') {
        messageEl.remove();
    }
}

function showRecallMessage(container, text, type) {
    if (!container) return;
    let note = container.querySelector('.mp-recall-message');
    if (!text) {
        if (note) note.remove();
        return;
    }
    if (!note) {
        ...
        note.className = 'mp-recall-message';
        container.appendChild(note);
    }
    note.dataset.type = type || 'info';
    note.textContent = text;
    note.style.marginTop = '8px';
    note.style.fontSize = '12px';
    note.style.lineHeight = '1.4';
    note.style.color = type === 'error' ? '#ff4d4f' : '#1f8a4c';
}

function setRecallButtonLoading(buttonEl, isLoading) {
    if (!buttonEl) return;
    if (!buttonEl.dataset.initialText) {
        buttonEl.dataset.initialText = buttonEl.textContent.trim() || 'Жду звонка';
    }
    if (isLoading) {
        buttonEl.disabled = true;
        buttonEl.textContent = 'Отправляем...';
        buttonEl.classList.add('mp-recall-submit--loading');
    } else {
        buttonEl.disabled = false;
        buttonEl.textContent = buttonEl.dataset.initialText;
        buttonEl.classList.remove('mp-recall-submit--loading');
    }
}
